...
angular.module('miles2run-home')
    .controller('NotificationsCtrl', function ($scope, $http, activeProfile, ConfigService) {
        ...
        $scope.notificationsPromise =  $http.get(ConfigService.getBaseUrl() + 'profiles/' + activeProfile.username + "/notifications").success(function (data, status, headers, config) {
